...
// 缩进/行间距识别出段落和标题，拼成一整本书的内容，再按目标字号和容器
// 尺寸重新分页——所以每页放多少字完全取决于设定的字号和容器尺寸，不会
// 因为塞进原始 PDF 的排版而变得太小或太挤。分页用的是离屏 canvas 估算
// 每段文字占几行（不是真的往 DOM 里插测量，见 paginateBlocks 那段专门
// 的性能取舍说明），所以估出来的高度和真实渲染之间会有些许误差，靠
// 安全余量兜底，不追求逐像素精确对齐。
const ReflowReader = (() => {
